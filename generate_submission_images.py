...
def generate_submission_images(generator_path: str, output_dir: str, num_images: int = 8000, batch_size: int = 64, latent_size: int = 128):
    ...
    print(f"🖥️  Using device: {device_info.backend}")
    ...
    try:
        state_dict = torch.load(generator_path, map_location=device_info.backend)
        generator.load_state_dict(state_dict)
        print(f"✅ Loaded generator from {generator_path}")
    except Exception as e:
        print(f"❌ Failed to load generator: {e}")
        print(f"   Make sure the model was trained with latent_size={latent_size}")
    ...
    if not (7000 <= num_images <= 10000):
        print(f"⚠️  Warning: Generating {num_images} images (recommended: 7,000-10,000)")
    ...
    with torch.no_grad():
        for batch_idx in tqdm(range(num_batches), desc="Generating batches"):
            ...
            z = torch.randn(current_batch_size, latent_size, 1, 1, device=device_info.backend)
    ...
    print(f"🏷️  File naming: 00001.jpg to {total_generated:05d}.jpg")
    ...
    print(f"\n🎯 Ready for submission! Total files: {len(list(Path(output_dir).glob('*.jpg')))}")


def main():
    parser = argparse.ArgumentParser(description="Generate Monet-style images for submission")
    ...
    if not (7000 <= args.num_images <= 10000):
        print("⚠️  Warning: num_images should be between 7,000 and 10,000 for submission")
