...
class EDA:
    ...
    def run_full_analysis(self):
        ...
        logger.log(f"EDA analysis complete! Results saved to: {self.output_dir}")
    ...
    def analyze_dataset_basics(self):
        ...
        with open(self.output_dir / "dataset_stats.txt", "w") as f:
            ...
            f.write(f"Image Extensions: {', '.join(stats['image_extensions'])}\n")
            f.write(f"Images per batch: {len(self.dataset) // self.params.batch_size}\n")
    ...
    def analyze_image_properties(self):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "image_properties.png", dpi=150, bbox_inches='tight')
        plt.close()
        
        logger.log(f"Image properties analysis complete (sampled {sample_size} images)")
    ...
    def analyze_pixel_distributions(self):
        ...
        for i in range(sample_size):
            img_tensor = self.dataset[i]  # This applies transforms (normalized to [-1, 1])
        ...
        # RGB histograms
        axes[0, 0].hist(all_pixels_r, bins=50, alpha=0.7, color='red', label='Red')
        axes[0, 0].hist(all_pixels_g, bins=50, alpha=0.7, color='green', label='Green')
        axes[0, 0].hist(all_pixels_b, bins=50, alpha=0.7, color='blue', label='Blue')
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "pixel_distributions.png", dpi=150, bbox_inches='tight')
        plt.close()
        ...
        with open(self.output_dir / "pixel_stats.txt", "w") as f:
            ...
            f.write(f"Red Channel:   {stats['red_mean']:.2f} ± {stats['red_std']:.2f}\n")
            f.write(f"Green Channel: {stats['green_mean']:.2f} ± {stats['green_std']:.2f}\n")
            f.write(f"Blue Channel:  {stats['blue_mean']:.2f} ± {stats['blue_std']:.2f}\n")
        
        logger.log(f"Pixel distribution analysis complete (sampled {sample_size} images)")
    ...
    def analyze_color_distributions(self):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "color_analysis.png", dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def visualize_samples(self):
        ...
        for i, idx in enumerate(sample_indices):
            ...
            transformed_img = ((transformed_tensor + 1) * 127.5).clamp(0, 255).byte()
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "sample_grid.png", dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def analyze_image_quality(self):
        ...
        for i in range(sample_size):
            try:
                ...
                logger.log(f"Error processing image quality for {img_path}: {e}")
                continue
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "quality_analysis.png", dpi=150, bbox_inches='tight')
        plt.close()
        ...
        with open(self.output_dir / "quality_stats.txt", "w") as f:
            ...
            f.write(f"Sample size: {sample_size} images\n\n")
            f.write(f"Sharpness: {np.mean(sharpness_scores):.2f} ± {np.std(sharpness_scores):.2f}\n")
            f.write(f"Contrast:  {np.mean(contrast_scores):.2f} ± {np.std(contrast_scores):.2f}\n")
            f.write(f"\nSharpness range: {np.min(sharpness_scores):.2f} - {np.max(sharpness_scores):.2f}\n")
            f.write(f"Contrast range:  {np.min(contrast_scores):.2f} - {np.max(contrast_scores):.2f}\n")
        
        logger.log(f"Image quality analysis complete (sampled {sample_size} images)")
    ...
    def generate_summary_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            f.write("1. **Dataset Basics** - File counts, extensions, directory structure\n")
            f.write("2. **Image Properties** - Dimensions, file sizes, aspect ratios\n")
            f.write("3. **Pixel Distributions** - RGB channel analysis\n")
            f.write("4. **Color Analysis** - Brightness, saturation, dominant colors\n")
            ...
            f.write("- `image_properties.png` - Image dimensions and file size analysis\n")
            ...
            f.write("- Images are preprocessed with resize, center crop, and normalization\n")
            f.write("- Color palette reflects Monet's impressionist style\n")
            f.write("- Suitable for GAN training with current preprocessing pipeline\n\n")
            ...
            f.write("- Monitor training for mode collapse given artistic style consistency\n")
